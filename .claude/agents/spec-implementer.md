...
6. Open a pull request via `mcp__github__create_pull_request` (owner/repo from `git remote get-url origin`), based against `main`, or against the dependency's branch if you stacked in step 4 above:
   ...
   - Body: fill in `.github/pull_request_template.md` section by section, using bullets everywhere except Description, and removing the HTML comments and placeholders. The template is built so a human can review AI-written code quickly, so the parts the reviewer can't recover from the diff matter most:
     - **Task**: an **absolute** permalink to the task file, pinned to the commit you just pushed: `https://github.com/<owner>/<repo>/blob/<git rev-parse HEAD>/specs/<module>/tasks/<task-file>.md`. A relative link doesn't work in a PR body (GitHub resolves it to a compare page), and a branch link breaks once the branch is deleted after merge. Then, if the task has a `**GitHub Issue:**` number, `Closes #<issue>` in exactly that form so the workflow can link it and close it on merge.
     - **Description**: 1–2 plain sentences on what the PR does and why, written for someone who hasn't read the task file.
     - **Review focus**: sort every file you changed into 🔍 read carefully / 👀 skim / ⏭️ skip, and say *what* to check in the read-carefully ones (a function name, an edge case). Status flips, `CONVENTIONS.md` notes and baseline PNGs are normally skip.
     - **Assumptions / deviations from spec**: list every decision the spec or task didn't dictate. That includes extra styling or behavior, how you handled edge cases the spec was silent on, tests beyond the task's list, and anything the task leaves to a later task. Be honest and complete. This is the section reviewers rely on most. Write "None" only if it really is none.
     - **Tests**: map the task's `**Test:**` field to the real test names, mark extras *(extra)*, and give the command to run them.
     - **Evidence**: for UI tasks, list the Playwright visual-baseline PNGs you updated (GitHub's rich diff shows them before/after). You can't upload images through the API, so don't try. Otherwise "N/A — no UI change".
     - **Stacked on**: only if you stacked in step 4 above. Say which dependency PR needs to merge first and which files/commits are actually new to this task, since the reviewer needs that to know what to review now. Otherwise delete the section. The base you set is the base that sticks: `.github/workflows/link-and-track-pr.yml` briefly round-trips it through `main` to register the issue link and then restores it, so don't warn in the PR body about a permanent retarget.
