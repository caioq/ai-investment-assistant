...
- **Storing `RISCO`, `SETOR`, or `CATEGORIA`.** The exports carry all three, and `RISCO`'s values (`AAA`/`A`/`B`/`C`) are even valid `RiskRating` members — but risk rating and sector are per-user classification on `Holding`, owned by [portfolio](../portfolio/spec.md) and supplied by the user's own holdings CSV. A research house's opinion is not the user's, and a wallet the user doesn't hold has no `Holding` row to write to. `CATEGORIA` (Dividends' `GERAÇÃO`/`TRANSMISSÃO`/…) is a presentation grouping nothing consumes.
...
- [ ] `RISCO`, `SETOR`, `CATEGORIA`, `PRECO_ATUAL` and `VARIACAO` are not persisted anywhere, and uploading a wallet writes nothing to `Holding` (where the user's own sector and risk rating live, per [portfolio](../portfolio/spec.md)) or to any column of `Asset` beyond creating the row itself.
