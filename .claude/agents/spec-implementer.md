...
6. Open a pull request via `mcp__github__create_pull_request` (owner/repo from `git remote get-url origin`), based against `main`, or against the dependency's branch if you stacked in step 4 above:
   ...
   - Body: fill in `.github/pull_request_template.md` section by section, using bullets only and removing the HTML comments and placeholders. The template is built so a human can review AI-written code quickly, so the parts the reviewer can't recover from the diff matter most:
     - **Task**: a relative link to the task file and, if the task has a `**GitHub Issue:**` number, `Closes #<issue>` in exactly that form so the workflow can link it and close it on merge.
