...
4. When the agent finishes, report: what was implemented, the test that verifies it, and the worktree path/branch so the user can review the diff and merge it themselves. Never merge, push, or apply the changes to the main working tree automatically. If the task had a linked `**GitHub Issue:**`, the agent already moved its card to **In Review** on the GitHub Project — remind the user to put `Closes #<issue>` in the PR description so merging to `main` closes it.
