...
Keep `investmentStyle`/`riskRating` nullable. <!-- Superseded: this sentence originally continued "the spec is explicit that this module never writes them (they're set from the holdings UI)". The spec now makes this module their only writer, via the assets CSV in MARKET_DATA_US-5. The schema this task produced is unchanged and still correct — nullable is still right, since an Asset created by a holding or a wallet upload is unclassified until an assets CSV covers it — so this task stays Done. --> Do **not** add a `holdings Holding[]` back-relation on `Asset` — `Holding` doesn't exist yet and belongs to [portfolio](../../portfolio/spec.md), which adds both sides of that relation itself. The `@@unique([assetId, date])` and `@@unique([benchmark, date])` constraints matter beyond data hygiene: they're what make the backfill and benchmark jobs idempotent in `MARKET_DATA_US-2_T-2` and `MARKET_DATA_US-3_T-3`.
...
<!-- Correction (MARKET_DATA_US-5_T-4): the `InvestmentStyle` enum this task produced was missing `ETF`, present in the spec's Data Model since before this task shipped — a transcription gap this task's own "exactly as written" instruction should have caught, and its test never exercised (no assertion covers enum *membership*, only `RiskRating`'s *order*). `SMAL11` (spec AC "`SMAL11` imports with `investmentStyle: ETF`") and this repo's `assets-full.csv`/`assets-empty-cells.csv` fixtures (`MARKET_DATA_US-5_T-1`) both depend on `ETF` existing, so T-4 added migration `20260827135242_add_etf_investment_style` (`ALTER TYPE "InvestmentStyle" ADD VALUE 'ETF'`) to close the gap. This task stays `Done` — the models/tables/RiskRating order it was actually tested against are still correct — but a future schema task transcribing an enum from spec.md should double-check membership, not just order. -->
