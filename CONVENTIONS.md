...
- Every Prisma model gets an explicit `@@map("<snake_case_plural>")` so the physical table name is a lowercase plural (`users`, not Prisma's default PascalCase singular `User`) even though the model name itself stays PascalCase singular per Prisma convention — see `User` in `apps/api/prisma/schema.prisma` (first model added, by `AUTH_US-1_T-1`). Model field names still match the spec's Data Model block verbatim (camelCase, e.g. `passwordHash`); only the table-level mapping is added on top.
