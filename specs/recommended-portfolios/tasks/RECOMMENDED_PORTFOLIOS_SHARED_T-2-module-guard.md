...
**Status:** Done
**GitHub Issue:** #141 (caioq/ai-investment-assistant — created by /user-stories; this file is still the source of truth, the issue mirrors it for GitHub Projects)
**Depends on:** none
...
Apply the shared `AuthGuard` (`apps/api/src/auth/auth.guard.ts`) **at the controller class**, not per-handler, per `CONVENTIONS.md` → "Auth". Every row this module writes carries a `userId` that always comes from `req.user.id` and never from the request — guarding the class means a handler added later is protected by default rather than by someone remembering.
