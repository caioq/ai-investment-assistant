...
- **Storing `RISCO`, `SETOR`, or `CATEGORIA`.** The exports carry all three, and `RISCO`'s values (`AAA`/`A`/`B`/`C`) are even valid `RiskRating` members — but risk rating and sector are `Asset` columns owned by [market-data](../market-data/spec.md) and will be populated from a different source (likely the user's holdings CSV) in a later module. Writing them here would make a per-wallet opinion authoritative for shared master data, and two wallets disagreeing about a ticker's risk would silently race. `CATEGORIA` (Dividends' `GERAÇÃO`/`TRANSMISSÃO`/…) is a presentation grouping nothing consumes.
...
- [ ] `RISCO`, `SETOR`, `CATEGORIA`, `PRECO_ATUAL` and `VARIACAO` are not persisted anywhere, and uploading a wallet leaves `Asset.riskRating` and `Asset.sector` untouched.
