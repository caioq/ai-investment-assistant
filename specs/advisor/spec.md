...
- **Prompt** has three input blocks:
  1. User's portfolio as JSON: holdings (`quantity` and `avgPrice` from `Holding`; `ticker`, current value, and the `sector` / `subSector` / `investmentStyle` / `riskRating` classification from `Asset` — see [market-data](../market-data/spec.md)), allocation by sector/stock/style/rating, performance metrics. Classification is optional per asset: a ticker not yet covered by an assets CSV import has `null` for all four, so the prompt must read as a partially-classified portfolio rather than assuming every field is present.
  2. `AdvisorReport.rawText` if `advisorReportId` was passed (truncated to ~15k chars).
  3. The latest `RecommendedPortfolio` per wallet type (from [recommended-portfolios](../recommended-portfolios/spec.md)) as JSON: `{walletType, effectiveDate, holdings: [{ticker, targetWeightPct, limitPrice, sector, subSector, investmentStyle, riskRating}]}` — lets the model compare actual against suggested allocation and flag holdings where `Asset.currentPrice > limitPrice`. The four classification fields are read from the recommended holding's `Asset`, **not** from the user's own positions: `RecommendedHolding` points at `Asset`, which is the whole reason classification lives there. Without it a recommended ticker the user doesn't yet hold would arrive unclassified, and "how would buying this shift my sector mix" — the question the comparison exists to answer — would be unanswerable. Expect `null`s here too: a recommended ticker is only classified once an assets CSV covers it.
...
- [ ] The prompt built for a portfolio whose assets are entirely unclassified (no assets CSV imported) contains no `undefined`/`"null"` string artifacts, and the request still succeeds — classification is optional context, never a precondition.
- [ ] A recommended wallet holding a ticker the user does **not** hold still carries that ticker's `sector` and `riskRating` in the prompt, sourced from its `Asset` row.
