# T-1 (US-4): docker-compose Postgres services
...
Add root `docker-compose.yml` with two Postgres services: `db` (dev, matching `DATABASE_URL` from [T-3_US-1](./T-3_US-1-env-example.md)'s `.env.example`) and `db-test` (test, on a separate host port so it never collides with `db`), per spec Behavior Notes.
