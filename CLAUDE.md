...
This project is built spec-first, in three stages per module: **spec → user stories/tasks → implement**. See [`WORKFLOW.md`](WORKFLOW.md) for the full guide (how to use each command/skill/agent, a Mermaid diagram of the end-to-end flow, a walkthrough, best practices) — this section is just the reference summary. Each module has a directory under [`specs/`](specs/):
...
| [demo-seed](specs/demo-seed/spec.md)                           | project-setup, auth, market-data, portfolio, recommended-portfolios, advisor, data-sources |
