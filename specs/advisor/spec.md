...
- **Prompt** has three input blocks:
  1. User's portfolio as JSON: holdings (ticker and current value from `Asset`; quantity, avgPrice, and the user's own sector / subSector / investmentStyle / riskRating from `Holding` — see [portfolio](../portfolio/spec.md)), allocation by sector/stock/style/rating, performance metrics. Classification is optional per holding: a user who uploaded a positions-only CSV has `null` for all four, so the prompt must read as a partially-classified portfolio rather than assuming every field is present.
