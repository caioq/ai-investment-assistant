...
Add the `User` model to `apps/api/prisma/schema.prisma` exactly as specified in the spec's Data Model section (`id` cuid, `email` unique, `passwordHash`, optional `name`, `createdAt` defaulted to `now()`), and generate the migration.
